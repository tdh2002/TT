...
void set_drawarea_property( DRAW_AREA *p, guint type, guint mask)
{
	...
	GROUP *p_grp = get_group_by_id (pp->p_config, p->group);
        gint grp = p->group ;
        p->scan_type = type;
	...
	switch (LAW_VAL_POS(p->group,Focal_type))
	{
		case AZIMUTHAL_SCAN:
							...
							else /*除了A scan 需要旋转的那种情况以外*/
							{
								...
								else
								{
									if(get_unit(pp->p_config) == UNIT_MM)
									{
										...
										p->wmin1 = GROUP_VAL_POS(p->group, start) * cos(TMP(current_angle[p->group]))/1000.0*(GROUP_VAL_POS(p->group, velocity)/200000.0);
										p->wmax1 = p->wmin1 + GROUP_VAL_POS(p->group, range) * cos(TMP(current_angle[p->group]))/1000.0*(GROUP_VAL_POS(p->group, velocity)/200000.0);
										p->w_unit = UNIT_MM;
									}
									else
									{
										...
										p->wmin1 = GROUP_VAL_POS(p->group, start)*cos(TMP(current_angle[p->group]))/1000.0*0.03937*(GROUP_VAL_POS(p->group, velocity)/200000.0);
										p->wmax1 = p->wmin1 + GROUP_VAL_POS(p->group, range)*cos(TMP(current_angle[p->group]))/1000.0*0.03937*(GROUP_VAL_POS(p->group, velocity)/200000.0);
										p->w_unit = UNIT_INCH;
									}
				...
				else /*除了A scan 需要旋转的那种情况以外*/
				{
					...
					else
					{
						if(get_unit(pp->p_config) == UNIT_MM)
						{
							...
							//p->wmax1 = get_group_val (p_grp, GROUP_RANGE)/1000.0*(get_group_val (p_grp, GROUP_VELOCITY)/200000.0)+get_group_val (p_grp, GROUP_START)/1000.0*(get_group_val (p_grp, GROUP_VELOCITY)/200000.0);
							p->wmin1 = GROUP_VAL_POS(p->group, start)/1000.0*(GROUP_VAL_POS(p->group, velocity)/200000.0);
							p->wmax1 = p->wmin1 + GROUP_VAL_POS(p->group, range) /1000.0*(GROUP_VAL_POS(p->group, velocity)/200000.0);
							p->w_unit = UNIT_MM;
						}
						else
						{
							...
							//p->wmax1 = get_group_val (p_grp, GROUP_RANGE)/1000.0*0.03937*(get_group_val (p_grp, GROUP_VELOCITY)/200000.0)+get_group_val (p_grp, GROUP_START)/1000.0*0.03937*(get_group_val (p_grp, GROUP_VELOCITY)/200000.0);
							p->wmin1 = GROUP_VAL_POS(p->group, start)/1000.0*0.03937*(GROUP_VAL_POS(p->group, velocity)/200000.0);
							p->wmax1 = p->wmin1 + GROUP_VAL_POS(p->group, range)/1000.0*0.03937*(GROUP_VAL_POS(p->group, velocity)/200000.0);
							p->w_unit = UNIT_INCH;
						}
						if(GROUP_VAL_POS(p->group, ut_unit) == UT_UNIT_TRUE_DEPTH)
							p->w_color = 0xD6ABF1;	/* 紫色 */
			...
			if(!pp->clb_flag)
			{
				if(GROUP_VAL_POS(p->group, ut_unit)==UT_UNIT_SOUNDPATH)
				{
					...
					if(LAW_VAL_POS(p->group,Focal_type) == AZIMUTHAL_SCAN)
					{
						...
						if(LAW_VAL_POS (p->group,Angle_min) == LAW_VAL_POS(p->group,Angle_max) )	/*hrule1*/	/* 灰色 */
						{
							p->hmin1 = LAW_VAL_POS (p->group, Angle_min) / 100.0;
							p->hmax1 = LAW_VAL_POS (p->group, Angle_max) / 100.0 + 1.0;
						}
					...
					else if (LAW_VAL_POS(p->group,Focal_type) == LINEAR_SCAN)
					{
						...
						p->hmin1 = (gfloat)(LAW_VAL_POS(p->group,First_tx_elem));
						//p->hmax1 = (gfloat)((( LAW_VAL (Last_tx_elem)-LAW_VAL(First_tx_elem) - LAW_VAL(Elem_qty) + 1 ) /
						//			LAW_VAL(Elem_step)) + 1);
						p->hmax1 = (gfloat)((( LAW_VAL_POS (p->group,Last_tx_elem)-LAW_VAL_POS(p->group,First_tx_elem)
				...
				else if(GROUP_VAL_POS(p->group, ut_unit) == UT_UNIT_TIME)
				{
					...
					if(LAW_VAL_POS (p->group,Angle_min) == LAW_VAL_POS(p->group,Angle_max) )	/*hrule1*/
					{
						p->hmin1 = LAW_VAL_POS (p->group, Angle_min) / 100.0;
						p->hmax1 = LAW_VAL_POS (p->group, Angle_max) / 100.0 + 1.0;
					}
				...
				else if(GROUP_VAL_POS(p->group, ut_unit) == UT_UNIT_TRUE_DEPTH)
				{
					...
					mid = (-1) * GROUP_VAL_POS(p->group,wedge.Primary_offset)/1000.0 - (GROUP_VAL_POS(p->group,probe.Elem_qty) * GROUP_VAL_POS(p->group,probe.Pitch)/1000.0)/2.0;
					...
					if((LAW_VAL_POS (p->group,Angle_min)<0)&&(LAW_VAL_POS (p->group,Angle_max)>0))
					{
					...
					else if((LAW_VAL_POS (p->group,Angle_min)<0)&&(LAW_VAL_POS (p->group,Angle_max)<0))
					{
					     p->wmin1 = middle + GROUP_VAL_POS(p->group, range)/1000.0 * sin((LAW_VAL_POS (p->group, Angle_min) / 100.0)*(3.14/180.0));
					     p->wmax1 = middle - GROUP_VAL_POS(p->group, range)/1000.0 * sin((LAW_VAL_POS (p->group, Angle_max) / 100.0)*(3.14/180.0));
					}
...
//							LAW_VAL(Elem_step)) + 1);
		...
		case TCG:
			...
			p->wmin1 = LAW_VAL_POS(p->group,Angle_min)/100;
			p->wmax1 = LAW_VAL_POS (p->group,Angle_max)/100;
			p->w_unit = UNIT_DEG;
			p->w_color = 0xCCD9D5;	/*浅灰色*/;
		...
		switch(pp->ctype_pos)
		{
			...
			case 1://Ultrasound
				if( !pp->cmode_pos )//Velocity
				{
					...
					if (LAW_VAL(Focal_type) == AZIMUTHAL_SCAN)
							set_drawarea_property (&(pp->draw_area[1]), S_SCAN_A, 0x0);
					else if (LAW_VAL(Focal_type) == LINEAR_SCAN)
							set_drawarea_property (&(pp->draw_area[1]), S_SCAN_L, 0x0);
				...
				else
				{
					...
					gtk_widget_show (pp->vbox_area[0]);
					if (LAW_VAL(Focal_type) == AZIMUTHAL_SCAN)
							set_drawarea_property (&(pp->draw_area[0]), S_SCAN_A, 0x06);
					else if (LAW_VAL(Focal_type) == LINEAR_SCAN)
							set_drawarea_property (&(pp->draw_area[0]), S_SCAN_L, 0x06);
					draw_area_ (pp->hbox_area[0], &(pp->draw_area[0]), 327, 212);
					set_drawarea_property (&(pp->draw_area[1]), A_SCAN, 0);
					draw_area_ (pp->hbox_area[0], &(pp->draw_area[1]), 327, 212);
					if( pp->cmode_pos == 1 )
					{
						set_drawarea_property (&(pp->draw_area[2]), WEDGE_DELAY, 0);
						draw_area_ (pp->vbox_area[0], &(pp->draw_area[2]), 655, 213);
					}
					else if( pp->cmode_pos == 2 )
					{
						set_drawarea_property (&(pp->draw_area[2]), SENSITIVITY, 0);
						draw_area_ (pp->vbox_area[0], &(pp->draw_area[2]), 655, 213);
					}
					else if( pp->cmode_pos == 3 )
					{
						set_drawarea_property (&(pp->draw_area[2]), TCG, 0);
						draw_area_ (pp->vbox_area[0], &(pp->draw_area[2]), 655, 213);
					}
						...
						case 4:
							...
							else if ((pp->ctype_pos == 1) && ((pp->cmode_pos == 0)||(pp->cmode_pos == 1)||(pp->cmode_pos == 2)||(pp->cmode_pos == 3)))
							{
								...
								if ((pp->pos_pos == MENU3_PRESSED) && (CUR_POS == 2))
								{
									...
									{
											cur_value = GROUP_VAL(gate[0].start) / 1000.0 ;   /* us */
											lower = 0.0;//BEAM_INFO(0,beam_delay) /1000.0;
											upper =	MAX_RANGE_US - GROUP_GATE_POS(width) / 1000.0;
											step = tmpf;
											digit = 2;
											pos = 2;
...
void calc_measure_data()
{
	gint offset,k;
	gint l,n;
	gfloat gate_data[setup_MAX_LAW_QTY][8];
	...
	{
		gate_data[index][1] = 
				(gfloat)(((TMP(measure_data[index][1])) & 0xfffff)*10.0 - TMP(max_beam_delay[grp]));
		gate_data[index][2] = 
				(gfloat)(((TMP(measure_data[index][2])) & 0xfffff)*10.0 - TMP(max_beam_delay[grp]));
		gate_data[index][3] = 
				(gfloat)(((TMP(measure_data[index][3])) & 0xfffff)*10.0 - TMP(max_beam_delay[grp]));
	}
	else if(LAW_VAL(Focal_point_type) == 1)//true depth
	{
		gate_data[index][1] = 
				(gfloat)(((TMP(measure_data[index][1])) & 0xfffff)*10.0 - TMP(max_beam_delay[grp])/cos(a));
		gate_data[index][2] = 
				(gfloat)(((TMP(measure_data[index][2])) & 0xfffff)*10.0 - TMP(max_beam_delay[grp])/cos(a));
		gate_data[index][3] = 
				(gfloat)(((TMP(measure_data[index][3])) & 0xfffff)*10.0 - TMP(max_beam_delay[grp])/cos(a));
	}
		...
		switch( DO_NOT_USE_CCFG(field[l]) )//field1
		{
			...
			case 6://A^
				if(GROUP_VAL(ut_unit)==1)//Time
					DO_NOT_USE_CCFG(measure_data[index]).a_position = gate_data[index][1]/1000;
				else
					DO_NOT_USE_CCFG(measure_data[index]).a_position = 
						(gate_data[index][1])*get_group_val (p_grp, GROUP_VELOCITY)/200000000.0;//距离模式s= time * velo/2
				TMP(field[l]) = DO_NOT_USE_CCFG(measure_data[index]).a_position;
				break;
			case 7://B^
				if(GROUP_VAL(ut_unit)==1)
					DO_NOT_USE_CCFG(measure_data[index]).b_position = gate_data[index][2]/1000;
				else
					DO_NOT_USE_CCFG(measure_data[index]).b_position = 
						(gate_data[index][2])*get_group_val (p_grp, GROUP_VELOCITY)/200000000.0;
				TMP(field[l]) = DO_NOT_USE_CCFG(measure_data[index]).b_position;
				break;
			case 8://I/
				if(GROUP_VAL(ut_unit)==1)
					DO_NOT_USE_CCFG(measure_data[index]).i_position = gate_data[index][3]/1000;
				else
					DO_NOT_USE_CCFG(measure_data[index]).i_position = 
						(gate_data[index][3])*get_group_val (p_grp, GROUP_VELOCITY)/200000000.0;
			...
			case 31://DA^
				...
				switch(GROUP_VAL(ut_unit))	
				{
					...
					case 1://Time
						DO_NOT_USE_CCFG(measure_data[index]).a_sound_path = 
							(gate_data[index][1])*get_group_val (p_grp, GROUP_VELOCITY)/200000000.0;//Time(A^)*声速/2
						break;
					case 2://True Depth
						DO_NOT_USE_CCFG(measure_data[index]).a_sound_path = 
							(gate_data[index][1])*get_group_val (p_grp, GROUP_VELOCITY)/(cos(a)*200000000.0) 
				...
				if( n%2 == 0 )
				{
					DO_NOT_USE_CCFG(measure_data[index]).a_depth = (DO_NOT_USE_CCFG(measure_data[index]).a_sound_path -TMP(field_distance[i]))*cos(a) - n*thickness;
				}
				else
				{
			...
			case 32://DB^
				...
				if( n%2 == 0 )
				{
					DO_NOT_USE_CCFG(measure_data[index]).b_depth = 
						(DO_NOT_USE_CCFG(measure_data[index]).b_sound_path -TMP(field_distance[i]))*cos(a) - n*thickness;
				}
				else
				{
					DO_NOT_USE_CCFG(measure_data[index]).b_depth =  
			...
			case 33://SA^
				switch(GROUP_VAL(ut_unit))	
				{
					case 0://Sound Path
						DO_NOT_USE_CCFG(measure_data[index]).a_sound_path = 
							(gate_data[index][1])*get_group_val (p_grp, GROUP_VELOCITY)/200000000.0;
						break;
					case 1://Time
						DO_NOT_USE_CCFG(measure_data[index]).a_sound_path = gate_data[index][1]/1000;//=A^
						break;
					case 2://True Depth
						DO_NOT_USE_CCFG(measure_data[index]).a_sound_path = 
							gate_data[index][1]*get_group_val (p_grp, GROUP_VELOCITY)/(cos(a)*200000000.0) 
			...
			case 34://SB^
				switch(GROUP_VAL(ut_unit))	
				{
					case 0://Sound Path
						...
						break;
					case 1://Time
						DO_NOT_USE_CCFG(measure_data[index]).b_sound_path = gate_data[index][2]/1000;
						break;
					case 2://True Depth
						DO_NOT_USE_CCFG(measure_data[index]).b_sound_path = 
							(gate_data[index][2]) * get_group_val (p_grp, GROUP_VELOCITY)/(cos(a)*200000000.0) 
							+ TMP(field_distance[i]);//=DA^/cos(a)+distance			
						break;
				}
...
void init_ui(DRAW_UI_P p)				
{
	...
	gtk_box_pack_start (GTK_BOX (p->hbox211), p->vbox_draw_area, FALSE, FALSE, 0);
